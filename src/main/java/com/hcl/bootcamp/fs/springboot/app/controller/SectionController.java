...
public class SectionController {
	...
	public String welcome(@ModelAttribute("sectionForm") SectionForm sectionForm,BindingResult bindingResult,Model model) {
		if (logger.isInfoEnabled()){
			logger.info("SectionController screen22 GET" );
		}
		val x = m_SectionRepository.findAll();
//		if (logger.isInfoEnabled()){
//			logger.info("SectionController *Sections " + x.size() );
//		}
		...
		if ( x != null && x.size() > 0 ) {
			...
			model.addAttribute("seats", i_Section.getSeats());
//			if (logger.isInfoEnabled()){
//				logger.info("SectionController *Sections " + i_Section.getSeats() );
//			}
		}
		return "screen2";
	}
	...
	public String bookTickets(@ModelAttribute("sectionForm") SectionForm sectionForm, BindingResult bindingResult,Model model) {
		if (logger.isInfoEnabled()){
			logger.info("SectionController booktickets POST" );
		}		
		...
		if (i_Section.isPresent()) {
			...
			for (Seat i_Seat  : i_SeatList) {
				if (logger.isInfoEnabled()){
					logger.info("SectionController i_Seat  " + i_Seat.getName() + " " + i_Seat.getId() + " " + i_Seat.getUserName() );
					if ( i_SeatID == i_Seat.getId() ) {
